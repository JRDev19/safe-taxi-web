...
const columns =
    [
        ...
        { data: null, render: function (data) {
            return `
                <div>
                    <a class="btn" href="${route('menus.show', { id: data.id })}">Ver</a>
                    <a class="btn" href="${route('menus.edit', { id: data.id })}">EDIT</a>
...
<template>
    <AppLayout title="Dashboard - Menús">
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg px-6 py-10">
                    <h1 class="text-2xl font-bold">Listado de Menús</h1>

                    <div class="relative float-right z-10 top-[35px]">
                        <a type="button" :href="route('menus.create')"> Agregar </a>
                        <a type="button" :href="route('menus.trash')"> Eliminados </a>

                    </div>
                    <DataTable :data="menus" :columns="columns" :options="options" class="display nowrap">
                        <thead>
                            <tr>
                                <th>#</th>
                                <th>Menú padre</th>
