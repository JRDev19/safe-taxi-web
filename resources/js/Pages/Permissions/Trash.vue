...
window.restore = (id) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            router.patch(route('permissions.restore', id))
        }
    });
};
...
window.drop = (id) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            router.delete(route('permissions.drop', id))
        }
    });
};
...
const restoreAll = () => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            router.post(route('permissions.restoreAll'))
        }
    });
};
...
const dropAll = () => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            router.post(route('permissions.dropAll'))
        }
    });
};
...
<template>
    <AppLayout title="Dashboard - Permissions">

        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg px-6 py-10">
                    <h1 class="text-2xl font-bold">Listado de Permisos eliminados</h1>
